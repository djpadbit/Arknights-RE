...
- The script [create_login.py](https://github.com/djpadbit/Arknights-RE/blob/master/create_login.py) allows you to generate credentials to login to the GS using a Yostar account and an optional device ID, one will be generated automatically if not specified. Theses credentials are stored in a file called `creds.json` and allows the [auto_daily.py](https://github.com/djpadbit/Arknights-RE/blob/master/auto_daily.py) script to function
...
This API was mostly written by using info from doing a [mitm](#man-in-the-middle) of the game.

# Man-In-The-Middle

For the Yostar login unification update (v28.4.01) I wanted to get a good mitm setup done because the one I had before was kind of horrible.
So I settled on something that works pretty well, The official Android Studio AVD emulator rooted, with Frida and a patched mitmproxy.
The Android image I used was a Google Play Store one, specifically Android 11 (API 30) x86 on a Pixel 7 Pro. I then used [rootAVD](https://gitlab.com/newbit/rootAVD) by [newbit](https://gitlab.com/newbit) to root the virtual machine (which works amazingly, less than a couple of minutes to fully root the device and have magisk installed).
Then I installed [Magisk-Frida](https://github.com/ViRb3/magisk-frida) in Magisk to have a [Frida](https://frida.re/) server running at all times on the machine.
I specifically used version 16.6.6-1 to avoid some bugs with starting applications within Frida. (Some other nice modules for Magisk are [safetynet-fix](https://github.com/kdrag0n/safetynet-fix) and [AlwaysTrustUserCerts](https://github.com/NVISOsecurity/MagiskTrustUserCerts)).

After all this, the emulator is ready to be MITM'ed. I then use [frida-interception-and-unpinning](https://github.com/httptoolkit/frida-interception-and-unpinning) to launch the app with certifiate pinning removed and all traffic being redirected to [mitmproxy](https://mitmproxy.org/). Sadly this doesn't work with standard mitmproxy but does with [HTTP Toolkit](https://httptoolkit.com/) but i'm not gonna use because I hate it on a personnal level. So I found a way to fix mitmproxy to work with the Frida scripts, you need to send both the certificate for the current website and the certificate authority when starting the TLS session. This can be done by editing the lines [here](https://github.com/mitmproxy/mitmproxy/blob/73349436ac0ce8ff9f3e2317dfc01d9cdd4a9934/mitmproxy/addons/tlsconfig.py#L228-L232) (and also [here](https://github.com/mitmproxy/mitmproxy/blob/73349436ac0ce8ff9f3e2317dfc01d9cdd4a9934/mitmproxy/addons/tlsconfig.py#L397-L400) although not needed) to add the CA to the certificates being sent, so just adding `self.certstore.default_ca` to the list `extra_chain_certs` will work. Only this fix should work, although I haven't tested it as my mitmproxy has more modifications in regards to certificates to make them a little more complient to the standards.
